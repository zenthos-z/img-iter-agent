...
# 复用 cli 里的 OpenAI 兼容 LLM（最小实现）。延迟导入避免循环依赖。
...
class LoopRunner:
    ...
    def start(
        ...
        *,
        ...
        if existing:
            ...
            return self._continue_existing(loop_id, store, lb, sample_id)
        ...
        store = RunStore.open(loop_id, settings=settings)
        handle = LoopHandle(loop_id=loop_id, phase="running")
        with self._lock:
    ...
    def _continue_existing(self, loop_id: str, store: RunStore, lb, sample_id: str) -> str:
    ...
    def _run_first_round(self, settings, lb, store, sample_id, *, resume_existing: bool = False):
        def task() -> None:
            handle = self.get(store.run_dir.name)
            try:
    ...
    def _run_resume(self, handle: LoopHandle, decision: str):
        def task() -> None:
            try:
                state = handle.app.invoke(Command(resume=decision), config=handle.cfg)
                self._post_invoke(handle, state)
            except Exception as e:  # noqa: BLE001
