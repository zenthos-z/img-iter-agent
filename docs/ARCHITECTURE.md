...
## 2.6 评分校准闭环：多维评分 + 人机协同调权重（用户提出）

> 用户明确了评分机制：**重点是还原度（不是泛泛风格一致）**；先由多模态 agent 给**多维度**评分，
> 后续**人工评分**，通过**多轮比较反推各维度因子的合适权重**，从而**提高 agent 判断准确率**。
> 这是与生成迭代闭环(§2)并行的**第二个闭环**，且是离线/异步的。

### 2.6.1 多维度评分（Critic 初评）

Critic 不再给单一总分，而是按 benchmark 定义的 `score_dimensions`（§3.5.2）逐维度打分：

```
生成图 + 目标参考图 + 验收 checklist
   └─▶ Critic(多模态) 逐维度评分:
         color_match: 8.2 | brushstroke: 7.0 | composition: 8.5
         content_fidelity: 7.8 | artifact: 9.0
       还原度总分 = Σ(维度分 × 当前权重)   # 权重初始用 benchmark 先验, 后续被校准
```

- 还原度总分用**当前权重**加权；权重会随校准闭环更新（见 2.6.3）。
- 初评分连同维度明细写入轨迹 `trajectory.jsonl` 的 `critic_scores`（§3.5.3）。

### 2.6.2 人工复评（异步补）

- 人工对（部分或全部）生成图按**同样的维度**独立打分，存入 `runs/<id>/human_scores/`。
- 与 Critic 初评分**配对**——这就是校准的监督信号：哪里 Critic 偏高/偏低一目了然。
- 人工评分不阻塞生成闭环（异步补），攒够一批再做校准。

### 2.6.3 权重校准：反推最优因子权重

当积累了一定量的「Critic 初评 + 人工评分」配对后，离线求解**最优维度权重**：
