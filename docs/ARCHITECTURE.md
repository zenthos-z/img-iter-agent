...
## 2.6 评分校准闭环：二分判定 + 人机协同校准（用户提出）

> 用户明确了评分机制并做了关键修正：**重点是还原度（不是泛泛风格一致）**；且
> **大模型擅长分类、不擅长量化**——所以评分从"多维度打 0-10 分"改为"**逐项二分判定（✓/✗）**"。
> 先由多模态 agent 做二分初判，后续**人工对同样项二分复判**，通过多轮比较**反推各项判定规则的准确度
> 与各维度的合适权重**，从而**提高 agent 判断准确率**。完整流程图见 `docs/EVALUATION.md`。
> 这是与生成迭代闭环(§2)并行的**第二个闭环**，且是离线/异步的。

### 2.6.1 二分判定（Critic 初判）

Critic 按 benchmark 的 `checklist`（每道考题 content_spec.json 定义）**逐项 ✓/✗ 判定**，
而非给 0-10 分：

```
生成图(三视图) + 目标参考图 + checklist
   └─▶ Critic(多模态) 逐项二分判定(每项带一句理由):
         C1 三视图是同一产品? ✓   C2 颜色一致? ✓
         C3 侧视宽=正视宽?  ✗     C4 高度比例一致? ✓
         S1 结构齐全?       ✓     S4 床头仍弧形? ✓
         ... (见 docs/EVALUATION.md)
       维度通过率 = 通过项数 / 总项数
       还原度 = Σ(维度权重 × 维度通过率)   # 权重初始用 benchmark 先验, 后续被校准
```

- 二分判定**可复现**（同输入同判定，不像打分会漂移），归因精准（每项 ✗ 都有理由）。
- 判定结果写入轨迹 `trajectory.jsonl` 的 `critic_checks`（§3.5.3）。

### 2.6.2 人工复判（异步补）

- 人工对（部分或全部）生成图按**同样的 checklist 项**独立做 ✓/✗，存入 `runs/<id>/human_scores/`。
- 与 Critic 初判**逐项配对**——这就是校准的监督信号：哪项 LLM 判错最多一目了然。
- 人工判定不阻塞生成闭环（异步补），攒够一批再做校准。

### 2.6.3 校准：逐项一致率 + 维度权重

当积累了一定量的「Critic 初判 + 人工复判」配对后，离线做两件事：

```
① 逐项一致率: 哪些 checklist 项 LLM 判错最多 → 针对性改该项的判定描述/阈值/示例
② 维度权重优化: 用人工复判算各维度"真实重要性" → 回归调维度权重 wᵢ
   目标: 找 w 使 Σ_samples | Σ_dim(w_dim × 人工维度通过率) − 人工整体满意度 | 最小
产出: runs/<id>/calibrated_weights.json + analyses/weight_calibration/ 报告(含逐项错误率)
```

- 校准比量化版**干净得多**：从"调一个飘忽的分数"变成"修一条明确的判定规则 + 调权重"。
- 校准结果**回灌 Critic 的 checklist 定义与判定逻辑**，下一轮 Critic 更准。

```
目标: 找权重 w = (w_color, w_brush, ...) 使
      Σ_samples | Σ_dim(w_dim × critic_dim) − 人工总分 |  最小
方法: 线性回归 / 加权最小二乘（约束 Σw=1, w≥0）
产出: runs/<id>/calibrated_weights.json + analyses/weight_calibration/ 报告
```

- 校准出的权重**回灌 Critic 的提示词 / 评分逻辑**，下一轮 Critic 用新权重算还原度总分
  → **agent 判断准确率随人工标注积累而提升**（这正是用户要的）。
- 若发现某维度 Critic 与人工系统性偏差大，可在 Critic 提示词里加针对性校准说明。
...
### ADR-008: 目标=还原度；评分=多维初评+人工复评+权重校准双闭环
- **状态**：✅ 已采纳（用户 2026-07-30 确认）
- **背景**：用户明确系统目标是**还原度**（生成图与目标的多维吻合），非泛泛"风格一致"；
  且评分机制是**人机协同校准**——agent 先多维初评，人工后续复评，多轮比较反推最优维度权重，
  提升 agent 判断准确率。此前架构只有单一 Critic 打分，缺校准闭环。
- **决策**：
  1. Critic 按 benchmark 的 `score_dimensions` **逐维度打分**，还原度总分=加权多维分（§2.6.1）；
  2. 人工按同维度**异步复评**，与初评配对作监督信号（§2.6.2）；
  3. 攒批后**回归求最优维度权重**，回灌 Critic 提示词/逻辑（§2.6.3）。
- **后果**：+评判从固定走向自我校准、人工标注越多越准；−需积累人工评分才能校准、
  离线流程需额外实现（pandas+sklearn）。
