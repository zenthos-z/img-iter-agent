...
| 谁写 | Summarizer 每轮写 | ExperienceDistiller（CLI distill）|
...
```bash
python -m img_iter_agent.cli distill --runs data/runs/loop-a data/runs/loop-b
```
...
- **清理测试 loop**：`data/runs/<loop_id>/` 整个目录删掉即可（含 sqlite checkpoint + trajectory + out 图）。带 tag 的测试 loop 跑完确认无用就删，别堆积污染 `data/runs/`、影响 analyze/distill 的输入。
...
- **测试 loop 混入蒸馏**：带 tag 的测试 loop 也在 `data/runs/` 下，会被 `distill --bench` 扫到。蒸馏重要经验前，先用 `--runs` 显式指定正式 loop，或先清理无用测试 loop。
